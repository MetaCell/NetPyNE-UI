...
#define m_inf m_inf_ical
 double m_inf = 0;
#define pcabar pcabar_ical
 double pcabar = 0.003;
#define tau_m tau_m_ical
 double tau_m = 0;
 ...
 "cai_ical", &cai_ical,
 "cao_ical", &cao_ical,
 "pcabar_ical", &pcabar_ical,
 "m_inf_ical", &m_inf_ical,
 "tau_m_ical", &tau_m_ical,
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 4);
 	...
 	ivoc_help("help ?1 ical /Users/matteocantarelli/Documents/Development/NEURON-UI/libs/models/PTCell/mod/x86_64/IL.mod\n");
...
static int  evaluate_fct (  double _lv ) {
 ...
 _ltadj = pow( 3.0 , ( ( celsius - 21.0 ) / 10.0 ) ) ;
   _la = 1.6 / ( 1.0 + exp ( - 0.072 * ( _lv + 5.0 ) ) ) ;
   _lb = 0.02 * ( _lv - 1.31 ) / ( exp ( ( _lv - 1.31 ) / 5.36 ) - 1.0 ) ;
...
static double _nrn_current(double _v){double _current=0.;v=_v;{ {
   ica = pcabar * m * m * ghk ( _threadargscomma_ v , cai , cao ) ;
   }
 _current += ica;
 ...
 v=_v;
{
 ...
 if(error){fprintf(stderr,"at line 62 in file IL.mod:\n	SOLVE states METHOD euler\n"); nrn_complain(_p); abort_run(error);}
