...
extern Memb_func* memb_func;
 ...
 "Exp1", _hoc_Exp1,
 "initrnd", _hoc_initrnd,
 "rates", _hoc_rates,
 0, 0
};
#define Exp1 Exp1_NMDAr
 extern double Exp1( double );
 /* declare global and static user variables */
#define Alpha Alpha_NMDAr
 double Alpha = 4;
#define Beta Beta_NMDAr
 double Beta = 0.0067;
#define Cdur Cdur_NMDAr
 double Cdur = 1;
#define Rtau Rtau_NMDAr
 double Rtau = 0;
#define Rinf Rinf_NMDAr
 double Rinf = 0;
#define mg mg_NMDAr
 double mg = 1;
#define usetable usetable_NMDAr
 double usetable = 1;
 ...
 "mg_NMDAr", &mg_NMDAr,
 "Cdur_NMDAr", &Cdur_NMDAr,
 "Alpha_NMDAr", &Alpha_NMDAr,
 "Beta_NMDAr", &Beta_NMDAr,
 "Rinf_NMDAr", &Rinf_NMDAr,
 "Rtau_NMDAr", &Rtau_NMDAr,
 "usetable_NMDAr", &usetable_NMDAr,
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 pnt_receive_size[_mechtype] = 5;
 	...
 	ivoc_help("help ?1 NMDAr /Users/matteocantarelli/Documents/Development/NEURON-UI/libs/models/PTCell/mod/x86_64/NMDAr.mod\n");
...
static double _hoc_initrnd(void* _vptr) {
 double _r;
    _hoc_setdata(_vptr);
 _r = 1.;
 initrnd (  );
...
static double _hoc_Exp1(void* _vptr) {
 ...
 static void _check_rates() {
  ...
  if (_maktable) { double _x, _dx; _maktable=0;
   _tmin_rates =  - 100.0 ;
   _tmax =  80.0 ;
   _dx = (_tmax - _tmin_rates)/180.; _mfac_rates = 1./_dx;
   for (_i=0, _x=_tmin_rates; _i < 181; _x += _dx, _i++) {
    _f_rates(_x);
    _t_B[_i] = B;
   }
   _sav_mg = mg;
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  release();
 if(error){fprintf(stderr,"at line 67 in file netrand.inc:\n  SOLVE release METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
...
static void _initlists() {
 ...
 _slist1[1] = &(Roff) - _p;  _dlist1[1] = &(DRoff) - _p;
   _t_B = makevector(181*sizeof(double));
_first = 0;
}
