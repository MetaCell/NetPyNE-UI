...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 4);
 	...
 	ivoc_help("help ?1 kv /Users/matteocantarelli/Documents/Development/NEURON-UI/libs/models/PTCell/mod/x86_64/kv.mod\n");
...
static int  rates (  double _lv ) {
   a = trap0 ( _threadargscomma_ _lv , tha , Ra , qa ) ;
   b = trap0 ( _threadargscomma_ _lv , tha , - Rb , - qa ) ;
   ntau = 1.0 / tadj / ( a + b ) ;
   ninf = a / ( a + b ) ;
    return 0; }
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  states();
 if(error){fprintf(stderr,"at line 74 in file kv.mod:\n  SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
